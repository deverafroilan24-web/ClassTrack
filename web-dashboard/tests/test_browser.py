...
def test_admin_add_edit_delete_on_desktop_and_mobile(browser_server):
    ...
    with playwright.sync_playwright() as p:
        ...
        row.get_by_role('button', name='Delete access', exact=True).click()
        page.get_by_role('button', name='Delete access', exact=True).last.click()
        playwright.expect(page.locator('#teacher-admin-message')).to_contain_text('access deleted')
        assert not errors, errors
        browser.close()
